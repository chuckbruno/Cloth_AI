...
- 2026-05-14: Implemented Phase 4.11 persistent contact patch + debug gizmos. `GoalNetSimulator` now has `enablePersistentContactPatch`, `contactPatchLifetime`, `contactPatchExtraRadius`, `contactPatchProjectionStrength`, and `contactPatchReactionScale`. Direct particle contacts, swept particle contacts, and swept edge endpoint contacts refresh a per-particle contact lifetime. Recent patch particles only apply soft projection when they are still inside the solver sphere, and otherwise only keep pocket pressure active while near the sphere, avoiding a sticky pull-back toward the ball. Realtime defaults: lifetime 4 FixedUpdates, extra radius 0.08m, projection strength 0.45, reaction scale 0.12. High-quality defaults: lifetime 5, extra radius 0.1m, projection 0.5, reaction 0.15. Added debug toggles `drawContactPatchGizmos`, `drawSweptPathGizmos`, and `drawGroundGizmos`; selected-object gizmos now show contact patch particles, swept path/spheres, contact-patch shell, and optional ground plane.
- 2026-05-14: Added preset organization and a simulator debug inspector. `GoalNetSimulator` now exposes read-only runtime stats for particle/edge/bending counts, ball speed, direct contacts, swept particle contacts, swept edge contacts, persistent patch counts, bottom tether count, and last applied ball impulse. Added context menu presets `Apply Stable 45mps Catch Settings`, `Apply High Speed 50mps Catch Settings`, `Apply Loose Net Pile Settings`, and `Apply Debug Visualization Settings`, alongside existing realtime/high-quality presets. Added `Editor/GoalNetSimulatorEditor.cs`, a custom Inspector that keeps the normal serialized fields but adds top preset buttons plus a live Runtime Debug panel.
